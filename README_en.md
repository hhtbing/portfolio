# Portfolio - Project Repository

> [**中文**](./README.md) | English

This is Benson's project portfolio repository, showcasing practical projects across **AI Platform / IoT Embedded / Mobile / Web Full-Stack / Tool Development / Data Services** and more technical domains.

## 📂 Repository Organization

This repository is organized with **Chinese-first, bilingual cross-linking**:

- Each project directory contains `README.md` (Chinese, default display) and `README_en.md` (English version)
- All READMEs have "Back to Profile" links and language switch links at the top
- Uses GitHub native Markdown rendering, no additional HTML pages needed

## 🗂️ Project Categories

### 🤖 AI & Platform
- [AgentMeshOS](./projects/01-ai-platform/agentmeshos) - Self-Hosted AI Agent Platform
- [Codex Host Bridge](./projects/01-ai-platform/codex-bridge) - Remote Development Bridge

### 📱 Mobile Applications
- [OwlCare](./projects/02-mobile/owlcare) - Smart Monitoring System (Android App + Config Tools + OTA Backend)

### 🔬 IoT & Embedded
- [OTA-QL](./projects/03-iot-embedded/ota-system) - Radar OTA Management System
- [AWRL6844 Radar Gateway](./projects/03-iot-embedded/radar-gateway) - ESP32 Radar Gateway
- [TI Radar Development Platform](./projects/03-iot-embedded/ti-radar-dev) - Historical Reference

### 🌐 Web & Business Systems
- [Owl Website](./projects/04-web-business/owl-website) - Corporate Website System ([https://www.wisefido.com/](https://www.wisefido.com/))
- [OwlFront Mock](./projects/04-web-business/owl-front-mock) - Frontend Demo System ([https://demo.wisefido.com/](https://demo.wisefido.com/))
- [Codex Mobile Control](./projects/04-web-business/codex-mobile-control) - Mobile Control

### 🔧 Tools & Utilities
- [DIR-MOVE-TOOLS](./projects/05-tools/dir-move-tools) - Directory Migration Tool
- [USB-MIC-MUTE](./projects/05-tools/usb-mic-mute) - USB Microphone Control
- [HEIC-TOOLS](./projects/05-tools/heic-tools) - Image Format Conversion
- [PDF-MD-TOOLS](./projects/05-tools/pdf-md-tools) - Document Conversion Tool
- [Icon-data](./projects/05-tools/icon-data) - Icon Data Management

### 📊 Data & Healthcare IT
- [FHIR Coding Dictionary](./projects/06-data/fhir-coding-dictionary) - Medical Coding Dictionary

### 🎮 Other Projects
- [Game Snake](./projects/07-others/game-snake) - WeChat Mini Game
- [Music MuseScore](./projects/07-others/music-musescore) - BGM Batch Generator

## 🚀 Access Methods

**From Profile:**
https://github.com/hhtbing → Click project links → View project details

**Browse Repository Directly:**
https://github.com/hhtbing/portfolio → Select project directory → View README

## 📖 Documentation

- Each project's README contains complete project overview, tech stack, core features, etc.
- Chinese version is the default display, English version accessible via top link
- All READMEs have "🏠 Back to Profile" link to return to the main profile

## 🔗 Related Links

- **Profile:** https://github.com/hhtbing
- **Repository:** https://github.com/hhtbing/portfolio

---

*Building practical systems with AI-assisted development (Vibe Coding), focused on solving real problems.*
